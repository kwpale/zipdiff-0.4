...
public class DifferenceCalculator {
    ...
    public boolean isCompareCompressedSizes() {
    ...
    protected Differences calculateDifferences(ZipFile zf1, ZipFile zf2) throws java.io.IOException {
        ...
        return calculateDifferences(zf1, zf2, map1, map2);
    }
     ...
     */
    protected Differences calculateDifferences(ZipFile zf1, ZipFile zf2, Map m1, Map m2) {
        ...
        while (iterAllNames.hasNext()) {
            ...
            if (names1.contains(name) && (!names2.contains(name))) {
                ...
                if (!entriesMatch(zf1, zf2, entry1, entry2, name)) {
    ...
    private static List inputStreamToLines(InputStream is) {
     ...
     */
    protected boolean entriesMatch(ZipFile zf1, ZipFile zf2, ZipEntry entry1, ZipEntry entry2, String name) {
        boolean result;
        ...
        if (result && !entry1.isDirectory() && !isZipFile(entry1.getName())) {
            try {
                List<Delta> deltas = DiffUtils.diff(inputStreamToLines(zf1.getInputStream(entry1)), inputStreamToLines(zf2.getInputStream(entry2))).getDeltas();
